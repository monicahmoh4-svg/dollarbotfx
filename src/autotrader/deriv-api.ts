...
export class DerivAPI extends EventTarget {
    ...
    connect(): Promise<void> {
        ...
        this.connectPromise = new Promise<void>((resolve, reject) => {
            ...
            ws.onclose = () => { clearTimeout(timeout); this.connectPromise = null; this.handleClose(); };
    ...
    private handleMessage = (event: MessageEvent) => {
        ...
        if (data.msg_type === 'tick' && data.tick) {
            const tick: DerivTick = { symbol: data.tick.symbol, quote: Number(data.tick.quote), epoch: Number(data.tick.epoch), id: data.tick.id };
            this.tickListeners.forEach(listener => { try { listener(tick); } catch (error) { console.error(error); } });
        }
        if (data.msg_type === 'proposal_open_contract' && data.proposal_open_contract) {
            this.pocListeners.forEach(listener => { try { listener(data.proposal_open_contract); } catch (error) { console.error(error); } });
        }
        if (data.req_id && this.waiters.has(data.req_id)) {
            const waiter = this.waiters.get(data.req_id)!;
            clearTimeout(waiter.timer);
            this.waiters.delete(data.req_id);
            if (data.error) waiter.reject(new Error(data.error.message || 'Deriv API error'));
    ...
    async send<T = any>(payload: Record<string, unknown>): Promise<T> {
        ...
        return new Promise<T>((resolve, reject) => {
            const timer = setTimeout(() => { this.waiters.delete(req_id); reject(new Error('Request timeout')); }, 30000);
            this.waiters.set(req_id, { resolve, reject, timer });
            this.ws!.send(JSON.stringify({ ...payload, req_id }));
        });
    }

    addTickListener(listener: (tick: DerivTick) => void) { this.tickListeners.add(listener); return () => { this.tickListeners.delete(listener); }; }
    addProposalOpenContractListener(listener: (poc: any) => void) { this.pocListeners.add(listener); return () => { this.pocListeners.delete(listener); }; }
    ...
    async activeSymbols(): Promise<DerivActiveSymbol[]> {
        const attempts: Array<{ label: string; payload: Record<string, unknown> }> = [
            { label: "full + basic", payload: { active_symbols: 'full', product_type: 'basic' } },
            { label: "brief + basic", payload: { active_symbols: 'brief', product_type: 'basic' } },
        ...
        for (const attempt of attempts) {
            ...
            } catch (error: any) { lastError = error instanceof Error ? error : new Error(String(error)); }
        }
    ...
    async getTickHistory(symbol: string, count = 300): Promise<DerivTick[]> {
        const response = await this.send({ ticks_history: symbol, adjust_start_time: 1, count, end: 'latest', style: 'ticks' });
        const prices = response?.history?.prices ?? [];
        const times = response?.history?.times ?? [];
        return prices.map((price: string | number, index: number) => ({ symbol, quote: Number(price), epoch: Number(times[index] ?? Date.now() / 1000) }));
    }
