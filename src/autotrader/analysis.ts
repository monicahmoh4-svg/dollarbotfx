...
const finiteQuotes = (quotes: number[]) => quotes.filter((quote) => Number.isFinite(quote) && quote > 0);
...
export function analyzeEvenOdd(input: number[], decimals: number): AnalysisResult {
  ...
  const predictEven = lastParity === 'odd';
  const predictOdd = lastParity === 'even';
  ...
  if (consecutiveStreak >= 4) score += f1w;
  else if (consecutiveStreak >= 3) score += f1w * 0.9;
  else if (consecutiveStreak >= 2) score += f1w * 0.7;
  else if (consecutiveStreak >= 1) score += f1w * 0.2;
  else penalties.push('NO_STREAK');

  const f2w = 20; maxScore += f2w;
  if (sameParityRatio10 >= 0.80) score += f2w;
  ...
  if (alternationRate >= 0.55) score += f3w;
  ...
  if (predictEven && longEvenRatio < 0.55) score += f6w;
