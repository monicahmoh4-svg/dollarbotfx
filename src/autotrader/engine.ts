...
// Quick-pick chips for the Symbol Override field — these are UI convenience
// only, not hardcoded trading logic. Whatever the user picks (or types) is
// still validated live against that session's real active_symbols and
// contracts_for results before anything is ever traded, so a stale or wrong
// code here just fails to match and is silently skipped — it can't cause a
// bad trade. Limited to the well-established, long-stable symbol codes
// (unchanged for years across the Deriv bot ecosystem) rather than guessing
// at less-certain ones (e.g. Boom/Crash/Jump code suffixes have varied).
...
class AutoTraderEngine extends EventTarget {
            ...
            this.settings = {
                ...
                enabledMarkets:
                    Array.isArray(saved.enabledMarkets) && saved.enabledMarkets.length
                        ...
                        : DEFAULT_AUTOTRADER_SETTINGS.enabledMarkets,
                tradeCategories:
    ...
    async start(patch: Partial<AutoTraderSettings> = {}) {
        ...
        try {
            const latencyMs = await this.api.ping();
            this.log('info', `Connection health check: ping ${latencyMs}ms.`);
        } catch (error: any) {
            this.log(
                'warn',
                `Ping failed (${error.message}). If active_symbols also comes back empty next, this points to a broad connectivity issue (network/proxy to wss://ws.derivws.com) rather than that call specifically.`
            );
        }
    ...
    private getSymbols(): DerivActiveSymbol[] {
        ...
        if (override.length) {
            ...
            // IMPORTANT: this used to just filter this.activeSymbols by the
            // override list — which does nothing useful when
            // this.activeSymbols is empty (e.g. the bulk active_symbols
            // call failed or returned empty), since filtering an empty
            // list by anything is still empty. Trading a specific,
            // explicitly-named symbol only actually needs per-symbol calls
            // (ticks_history, contracts_for, proposal) — none of which
            // depend on that one bulk discovery call succeeding — so an
            // override symbol not found in the last active_symbols
            // response is now tried directly instead of silently dropped.
            const unresolved = override.filter(code => !knownCodes.has(code));

            const synthesized: DerivActiveSymbol[] = unresolved.map(code => ({
                symbol: code,
                display_name: code,
                market: this.settings.enabledMarkets[0] ?? 'synthetic_index',
                exchange_is_open: 1,
            ...
            if (synthesized.length) {
                this.log(
                    'warn',
                    `${synthesized.length} Symbol Override entr${synthesized.length === 1 ? 'y' : 'ies'} (${unresolved.join(
                        ', '
                    )}) ${synthesized.length === 1 ? "wasn't" : "weren't"} in the last active_symbols list — trying ${
                        synthesized.length === 1 ? 'it' : 'them'
                    } directly via per-symbol calls instead of skipping.`
                );
            }
